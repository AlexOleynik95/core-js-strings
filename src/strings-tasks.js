...
function repeatString(str, times) {
  // handle cases for non-positive 'times' values
...
function removeFirstOccurrences(str, value) {
  const valueLastIndex = str.indexOf(value);
...
function isPalindrome(str) {
  ...
  return strFormatted.split('').reverse().join('') === strFormatted;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    // had to handle non-alphabetic characters along with lowercase
    // as eslint disallows using 'continue' when adding separate check for them
    // and without 'continue' JS throws an error as currentAlphabet remains undefined
    if (UPPERCASE_ALPHABET.includes(str[i])) {
      currentAlphabet = UPPERCASE_ALPHABET;
    } else {
      currentAlphabet = LOWERCASE_ALPHABET;
    }

    const letterIndex = currentAlphabet.indexOf(str[i]);
    const letterIndexShifted = letterIndex + 13;

    // screw you eslint
    if (!currentAlphabet.includes(str[i])) {
      result += str[i];
    } else if (letterIndex + 13 > currentAlphabet.length - 1) {
      result += currentAlphabet[letterIndexShifted - currentAlphabet.length];
    } else {
      result += currentAlphabet[letterIndexShifted];
    }
  }
...
function getCardId(value) {
  // '10' is treated as '1'
  const CARDS_RANKS = 'A234567891JQK';
  const CARDS_SUITS = '♣♦♥♠';
  let result = '';

  // ranks check reads card value from the end
  // to handle '10...' cases
  result +=
    CARDS_RANKS.indexOf(value[0]) +
    CARDS_SUITS.indexOf(value.at(-1)) * CARDS_RANKS.length;
